...
void WeddleRule() {
    ...
    float lower_limit, upper_limit, integration = 0.0, step_size, k, s0, s1{}, s2{},
        s3{}, s4{};
    int i, num_intervals;

    cout << "******************************************************" << endl;
    cout << "WEDDLE'S RULE" << endl;
    cout << "******************************************************" << endl;
    ...
    if (num_intervals % 6 == 0) {
        ...
        cout << "******************************************************" << endl;
        cout << setw(2) << "S.NO" << setw(20) << "lower_limit+i*h" << setw(30) <<
            "f(lower_limit+i*h)" << endl;
        cout << "******************************************************" << endl;
        ...
        for (i = 1; i <= num_intervals - 1; i = i + 6)
            s1 += 5 * (f(lower_limit + i * step_size) +
                f(lower_limit + (i + 4) * step_size));

        for (i = 3; i <= num_intervals - 3; i = i + 6)
            s2 += 6 * (f(lower_limit + i * step_size));

        for (i = 2; i <= num_intervals - 2; i = i + 6)
            s3 += (f(lower_limit + i * step_size) +
                f(lower_limit + (i + 2) * step_size));
